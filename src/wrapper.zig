...
pub fn Wrapper(comptime kind: Kind, comptime ReaderType: type, comptime WriterType: type) type {
    ...
    return struct {
        ...
        pub const Reader = switch (kind) {
            .raw => ReaderType,
            else => std.io.Reader(*Self, ReadError, read),
        };
        pub fn reader(self: *Self) Reader {
            switch (kind) {
                .raw => return self.rdr,
                else => return .{ .context = self },
            }
        }
        ...
        pub const Writer = switch (kind) {
            .raw => WriterType,
            else => std.io.Writer(*Self, WriteError, write),
        };
        ...
        inline fn update(self: *Self, buf: []const u8) void {
            switch (kind) {
                .raw => {},
                else => {
...
test "raw wrapper" {
    const data = "iso medo u ducan nije reko dobar dan";
    var fbs = std.io.fixedBufferStream(data);

    var w = init(.raw, fbs.reader(), std.io.null_writer);
    ...
    try testing.expect(w.bytes == 0);
    try testing.expect(w.bytesRead() == 0);
    try testing.expect(w.chksum() == 0);
}

pub fn Wrapper2(comptime kind: Kind, comptime WriterType: type) type {
    const HasherType = switch (kind) {
        .gzip => std.hash.Crc32,
        .zlib => std.hash.Adler32,
        .raw => NullHasher,
